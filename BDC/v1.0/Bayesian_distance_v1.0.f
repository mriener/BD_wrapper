      program Bayesian_distance_pdf

c     Generates a distance probability density function for a source
c     given its (ell,bee,Vlsr) values.

c     Uses spiral arm models, kinematic distances, nearby sources with parallax
c     measurement, and Galactic latitude inference.
...
c        probability_controls.inp  ! Weights for each of the 4 probability density components

c        parallax_data.inp         ! Parallax source data

c        sources_info.inp          ! Source name, longitude, latitude, Vlsr, P(far), extra_info

c                                  ! Individual arm (l,b,v, R, beta, D) traces
c        Out_lbvRBD.dat            ! Outer arm
c        Per_lbvRBD.dat            ! Perseus arm
c        Loc_lbvRBD.dat            ! Local Arm
c        CrN_lbvRBD.dat            ! Carina arm (near portion)
c        CrF_lbvRBD.dat            ! Carina arm (far portion)
c        SgN_lbvRBD.dat            ! Sagittarius arm (near portion)
c        SgF_lbvRBD.dat            ! Sagittarius arm (far portion)
c        CtN_lbvRBD.dat            ! Centaurus-Crux arm (near portion)
c        CtF_lbvRBD.dat            ! Centaurus-Crux arm (far portion)
c        ScN_lbvRBD.dat            ! Scutum arm (near portion)
c        ScF_lbvRBD.dat            ! Scutum arm (far portion)
c        OSC_lbvRBD.dat            ! Q1 and Q2 "Outer Scutum-Crux" arm
c        Nor_lbvRBD.dat            ! Norma arm
c        3kN_lbvRBD.dat            ! near 3-kpc arm
c        3kF_lbvRBD.dat            ! far 3-kpc arm
c        AqS_lbvRBD.dat            ! Aquila Spur
c        CnN_lbvRBD.dat            ! near Connecting arm
c        CnX_lbvRBD.dat            ! extension (bifurcation) of Connecting arm
c        AqR_lbvRBD.dat            ! Aquila Ridge
c        LoS_lbvRDB.dat            ! Local Spur
...
c        sourcename_spiral_arm_pdf.dat          ! spiral arm component of full pdf
c        sourcename_kinematic_distance_pdf.dat  ! kinematic distance component of full pdf
c        sourcename_latitude_pdf.dat            ! latitude component of full pdf
c        sourcename_parallaxes_pdf.dat          ! parallax-source association component of full pdf
...
c                                   component with the greatest integrated probability
      ...
      character*48  control_file, sources_file, srcprt_file
      ...
      real*8        prob_arm(1001), prob_Dk(1001), prob_lat(1001)
...
c     Arm information arrays
      real*8        arm_probabilities(20)
      real*8        arm_ell(20,300), arm_bee(20,300), arm_vel(20,300)
      real*8        arm_Rgc(20,300), arm_beta(20,300), arm_dist(20,300)
      real*8        seg_ell_min(20,2), seg_ell_max(20,2)
      real*8        d_store(20), b_store(20)
      integer       iarm_entries(20)
      character*12  a_store(20)

      character*48  arm_file, galaxy_file

      character*12  arm_assigned, arm_indicated, unknown
      character*12  a_name, arm_name(20), arm_max, arm_max_bin(1001)

      character*20  extra_info
      character*2   questionable
...
c     max_num_arms   = 20        ! max number of arm segments (hardwired in subroutine)
...
cc      write (lu_print,1000)
 1000 format(' Program: Bayesian_distance_pdf: April 5, 2016')
...
c     To calculate prob of being in an arm based on (l,b,v)-traces
      sig_vel     = 5.d0      ! km/s
      ell_dif_max = 5.d0      ! deg   maximum difference in longitude to bother calculating arm prob

c     To calculate probability based on arm width vs Radius
c     Arm width formula  W = 0.14 + (R-4)*0.042 kpc for R>4kpc
      width_min  = 0.14d0     ! kpc  (Gaussian sigma)
      width_Rref = 4.d0       ! kpc
      width_slope= 0.042d0    ! kpc/kpc

c     To calculate prob of being off the (warped) plane
c     Arm z-width formula  W_z = 0.04 + (R-8)*0.042 kpc for R>8kpc
      sigz    = 0.04d0        ! kpc      (1-sig z-height for Rgc<Rsigz)
      Rsigz   = 8.d0          ! kpc
      sigzdot = 0.042d0       ! kpc/kpc  (slope of z-height with Rgc)
...
c               Read in target sources for distance PDF...
      ...
      sources_file = 'sources_info.inp'    ! containing source information
c     Open file and count number of sources
      call open_ascii_file ( lu_sources, sources_file, lu_print )

cc      write (lu_print,1120)
 1120 format(/'   Long    Lat   Vlsr       Dist  sigma-D   Prob     ',
     +        'Arm     Extra Information',
     +       /'  (deg)   (deg) (km/s)     (kpc)   (kpc)',/)
...
c      Check for "commented-out" source line (ie, starting with a "!")
       ...
       if ( s1.ne.'!' .and. ieof.ge.0 ) then
         ...
         read (lu_sources,*) src,ell,bee,v_lsr,far_prob,extra_info
...
c        Strip blanks out of source name in output file naming
 ...
 1130    format(a,'.prt')
         ...
         write (lu_srcprt,1140) src,ell,bee,v_lsr,far_prob,extra_info
 1140    format(//'================================================',
     +      /'   Source       Long.   Lat.     Vlsr    P(F)  Extra_info',
     +      /a14,2f7.2,2x,f7.1,2x,f5.2,3x,a20,/1x)
...
c        Use (ell,z,vel) to assign arm segment probabilities
         call assign_arm_probabilities ( ell, bee, v_lsr,
     +            ell_dif_max,
...
c              Calculate differences between target and parallax source
               dell = abs( ell - ref_ell(n_p) )                  ! deg
               dbee = abs( bee - ref_bee(n_p) )                  ! deg
               dvel = abs( v_lsr-ref_vlsr(n_p))                  ! km/s

c              Convert (longitude,latitude) to (rho,zee) in kpc units,
c              using the parallax-source's measured parallax
               drho = dell*deg_to_rad / ref_par(n_p)             ! kpc
               dzee = dbee*deg_to_rad / ref_par(n_p)             ! kpc

c              Both the target and parallax sources have significant V
c              uncertainty, the V-difference uncertainty will be larger
               sig_vel_dif = sqrt(2.d0)*sig_vel
...
c        =============================================================
c        Now work towards a full Prob(d) by including Prob_SA, Prob_KD, Prob_GL...
...
c        Combine probabilities for arm, Dk, and latitude to make a
c        "Milky Way" model based pdf
         do n_ps = 1, num_bins
            prob_MWmodel(n_ps) = 0.d0
            if ( prob_arm(n_ps).gt.1.d-09 .and.
     +           prob_Dk(n_ps) .gt.1.d-09 .and.
     +           prob_lat(n_ps).ge.1.d-09      ) then
               prob_MWmodel(n_ps) = prob_arm(n_ps)*prob_Dk(n_ps)
     +                                            *prob_lat(n_ps)
            endif
         enddo
...
c        Find maximum probability distance
         p_int_max = 0.d0
         n_max     = 0
         if ( num_peaks .ge. 1 ) then
            do n_p = 1, num_peaks
...
c           Flag if we didn't fit Gaussians...
            ...
            write (lu_arm,3200) ell, bee, v_lsr,
     +          peak_dist(n_max),peak_dunc(n_max),peak_int(n_max),
     +          arm_indicated,questionable, extra_info
ccc            write (lu_print,3200) ell, bee, v_lsr,
ccc     +          peak_dist(n_max),peak_dunc(n_max),peak_int(n_max),
ccc     +          arm_indicated,questionable, extra_info
 3200       format(2f7.2,f7.1,3x,3f8.2,5x,a3,a2,3x,a)
...
c======================================================================
      ...
      subroutine assign_arm_probabilities ( ell, bee, v_lsr,
     +           ell_dif_max,
     +           num_arms, arm_ell, arm_bee, arm_vel, iarm_entries,
     +           arm_Rgc, arm_beta, arm_dist,
     +           width_min, width_Rref, width_slope,
...
c           now have arm information every 1 deg,
...
c              Interpolate in steps of 0.1 degrees of longitude
...
c                    Increase sig_vel to account for difference between clouds
c                    used to define the arm segment and the target source
                     sig_vel_dif = sqrt(2.d0) * sig_vel             ! km/s
...
c======================================================================
      ...
      subroutine possible_arms(num_arms, seg_ell_min, seg_ell_max, ell,
     +                         possible_arm, n_possible_arms)
      ...
      do n_a = 1, num_arms
         ...
         possible_arm(n_a) = .false.

         do i_r = 1, 2

c           define "possible" if within 4 degrees of arm_segment ranges...
            if ( ell.ge.(seg_ell_min(n_a,i_r)-4.d0) .and.
     +           ell.le.(seg_ell_max(n_a,i_r)+4.d0)      ) then
               possible_arm(n_a) = .true.
               n_possible_arms = n_possible_arms + 1
            endif
...
c     from the center of a given spiral arm
      ...
      real*8        arm_probabilities(20)
      real*8        d_store(20), b_store(20)
      real*8        arm_d_min(20), arm_b_min(20)
      ...
      character*12  arm_name(20), arm_max, atemp
      character*12  a_store(20), arm_name_b_min
...
c     the minimum separation point and the latitude at that point
      ...
      real*8   arm_probabilities(20)
      real*8   arm_b_min(20), arm_d_min(20)
...
c     P(d) = P(d|arm)*P(arm)
      ...
      character*12  arm_name(20), arm_max
...
c     This version avoids actually calculating the kinematic distance(s),
...
c     the near and far PDF components
      ...
      sig_vel_avg = sig_vel
      call asym_conservative_prob( V_mod, sig_vel, sig_vel_avg,
     +                             v_lsr_rev, prob )
...
c     Non-flat rotation curve used...
      call Univ_RC ( a1, a2, a3, Ro, Rs,
     +               Tr, To )
...
c      pmin   = 2.d-03/bin_size           ! minimum prob for a peak
      pmin   = 1.01d0/(num_bins*bin_size) ! set just above pure flat PDF

      pratio = 0.1d0                      ! max drop off next to peak
...
c     Arm information arrays
      ...
      character*12  a_name, arm_name(20)
      character*1   c1

c     Spiral arm segment names; used to point to (lbvRBD) files

      num_arms    = 20            ! current dimension limit is 20
      ...
      arm_name(12)= 'OSC'         ! "Outer Scutum-Crux" arm

      arm_name(13)= 'Nor'         ! Norma arm

      arm_name(14)= '3kN'         ! 3-kpc near side of GC
      arm_name(15)= '3kF'         ! 3-kpc far side

      arm_name(16)= 'AqS'         ! Aquila Spur

      arm_name(17)= 'CnN'         ! Connecting arm near side (of GC)
      arm_name(18)= 'CnX'         ! Connecting arm near side extension

      arm_name(19)= 'AqR'         ! Aquila Rift

      arm_name(20)= 'LoS'         ! Local spur
